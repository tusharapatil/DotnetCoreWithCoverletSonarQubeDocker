...
RUN dotnet sonarscanner begin /n:$projectName /k:$projectName \
	...
	/d:sonar.cs.vstest.reportsPaths="**/TestResults/testresult.xml" \ 
...
RUN dotnet test -c release -f netcoreapp2.2 --filter Category=Dev /p:CollectCoverage=true  /p:CoverletOutput=../../CoverageResults/ /p:MergeWith="../../CoverageResults/coverage.json" /p:CoverletOutputFormat=\"opencover,json\" --logger:"trx;LogFileName=testresult.xml" -m:1	
